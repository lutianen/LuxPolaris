...
const sockaddr*
sockets::sockaddr_cast(const sockaddr_in6* addr) {
    return static_cast<const sockaddr*>(implicit_cast<const void*>(addr));
}

sockaddr*
sockets::sockaddr_cast(sockaddr_in6* addr) {
    return static_cast<sockaddr*>(implicit_cast<void*>(addr));
}

const sockaddr*
sockets::sockaddr_cast(const sockaddr_in* addr) {
    return static_cast<const sockaddr*>(implicit_cast<const void*>(addr));
}

const sockaddr_in*
sockets::sockaddr_in_cast(const sockaddr* addr) {
    return static_cast<const sockaddr_in*>(implicit_cast<const void*>(addr));
}

const sockaddr_in6*
sockets::sockaddr_in6_cast(const sockaddr* addr) {
...
void
sockets::bindOrDie(int sockfd, const sockaddr* addr) {
...
/// @return
int
sockets::accept(int sockfd, sockaddr_in6* addr) {
 ...
 */
int
sockets::connect(int sockfd, const sockaddr* addr) {
 ...
 */
ssize_t
sockets::readv(int sockfd, const iovec* iov, int iovcnt) {
...
void
sockets::toIpPort(char* buf, size_t size, const sockaddr* addr) {
    toIp(buf, size, addr);
...
void
sockets::toIp(char* buf, size_t size, const sockaddr* addr) {
...
void
sockets::fromIpPort(const char* ip, uint16_t port, sockaddr_in* addr) {
...
void
sockets::fromIpPort(const char* ip, uint16_t port, sockaddr_in6* addr) {
...
bool
Socket::getTcpInfo(struct tcp_info* tcpInfo) const {
...
/**
 * @brief 接收方得满足（不通告小窗口给发送方） + 发送方开启 Nagle 算法，就可以避免糊涂窗口综合症.
 * Nagle 算法默认开启，然而对于一些需要小数据包交互的场景的程序，则需要关闭 Nagle 算法.
 * Solution: 在 Socket 设置 @c TCP_NODELAY 选项关闭 Nagle 算法.
 */
void
