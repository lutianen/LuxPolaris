...
struct regression1_type {
    ...
    regression1_type&
    operator=(const regression1_type&) {
        return *this;
    }
    regression1_type&
    operator=(regression1_type&&) {
        return *this;
    }
};
...
main() {
...

    {
        ...
        CHECK(Lux::any_cast<big_type>(big1).check() && Lux::any_cast<big_type>(big2).check() && Lux::any_cast<big_type>(big3).check());
    }
...

    {
        ...
        Lux::any i = 400;
        Lux::any f = 400.0f;
        // Lux::any unique = std::unique_ptr<big_type>(); -- must be copy constructible
        ...
        // CHECK(is_stack_allocated(unique, Lux::any_cast<std::unique_ptr<big_type>>(&unique)));
        CHECK(is_stack_allocated(shared, Lux::any_cast<std::shared_ptr<big_type>>(&shared)));
        ...
        CHECK(is_stack_allocated(r1, Lux::any_cast<const regression1_type>(&r1)));
    }
